...
class QueryAnalyzer {
  ...
  val filterSelections: (Field, Map[String, List[AnalyzedField]]) => immutable.Iterable[(Field, QueryType)] = (field, subfields) =>
    field.selections
  ...
  def getQueryTypes[Ctx, Val](schema: Schema[Ctx, Val]): Map[String, OutputType[_]] =
    schema.query.fields.map(x => (x.name, x.fieldType)).toMap
  ...
  def splitQuery[Ctx, Val](field: Field, types: Map[String, OutputType[_]]): List[(Selection, Set[QueryType])] = {
    ...
    val innerSelection =
      rootSelection.map(extractSubfields(_, field.selections.map { case FieldNameExtractor(name) => name }.toList)).getOrElse(Map.empty)
    val innerSelectionTypes: List[AnalyzedField] = innerSelection.map { case (name, field) =>
      AnalyzedField(name, field, resolveType(field))
    }.toList
    ...
    val simpleSubQuery =
      if (simpleSelection.isEmpty) None
      else Some(field.copy(selections = simpleSelection.map(_._1).toVector), simpleSelection.map(_._2).toSet)
    ...
    val complexSubQuery: Seq[Some[(Field, Set[QueryType])]] = complexSelection
      ...
      .flatMap { case (value, queryType) =>
        value.map { case (selection, types) => Some(field.copy(selections = Vector(selection)), types + queryType) }
      }
      .toList
  ...
  def mergeFields(fields: Vector[Selection]): Field =
    fields.collect { case a: Field => a }.reduce((a, b) => a.copy(selections = a.selections ++ b.selections))
