...
class Person(tag: Tag) extends slick.jdbc.H2Profile.api.Table[PersonRecord](tag, "Person") {
  ...
  def cityFk = foreignKey("cityFk", cityId, City.table)(_.id, onUpdate = ForeignKeyAction.Restrict, onDelete = ForeignKeyAction.Cascade)
