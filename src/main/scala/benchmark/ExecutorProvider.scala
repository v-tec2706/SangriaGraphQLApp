...
object ExecutorProvider {
  ...
  def provide(strategy: Strategy): Execution = l
    .find(_.strategy(strategy))
    .getOrElse(StandardExecutor(MainResolver.build, QueriesSchema.asyncSchema, None, Strategies.Async))
}
