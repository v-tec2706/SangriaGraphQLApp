...
object AsyncEntities {
        ...
        Field(
          ...
          resolve = ctx => {
            for {
              friendIds <- ctx.ctx.personResolver.knows(ctx.value.id)
              workAt <- Future
                .sequence(friendIds.map(ctx.ctx.companyResolver.worksAt))
              ...
              companyLocatedIn <- Future
                .sequence(workAt.map(x => ctx.ctx.companyResolver.getCompany(x._2)))
  ...
  def personResolver(env: MainResolver): EntityResolver[MainResolver, Json] = EntityResolver[MainResolver, Json, Person, PersonArg](
    __typeName = "Person",
