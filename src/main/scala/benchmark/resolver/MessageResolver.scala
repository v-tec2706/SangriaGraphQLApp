...
object MessageResolver {
  ...
  val batchedMessageResolver: Fetcher[MainResolver, Message, Message, Long] =
    Fetcher((ctx: MainResolver, ids: Seq[Long]) => ctx.messagesResolver.getMessages(ids.toList))
  val cachedMessageResolver: Fetcher[MainResolver, Message, Message, Long] = Fetcher(
    ...
    fetch = (ctx: MainResolver, ids: Seq[Long]) => {
      val seq: Seq[DBIOAction[Message, NoStream, Effect.All]] =
        ids.map(id => ctx.messagesResolver.messagesRepository.getMessage(id).map(_.head))
